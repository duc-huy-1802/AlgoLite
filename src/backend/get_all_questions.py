    ...
    try:
        if (event.get("httpMethod") != "GET"):
            return make_response(405, {
                "error": "Method not allowed. Use GET /questions."
            })

        question_list = get_questions()
        if not question_list:
            return make_response(404, {"error": "no questions found"})

        return make_response(200, {"questions": question_list})
...
def get_questions() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        response = questions_table.scan(
            ProjectionExpression="problem_id, title"
        )
